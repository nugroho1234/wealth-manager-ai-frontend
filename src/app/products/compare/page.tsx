  ...
  const fetchComparisonData = async (productIds: string[]) => {
    try {
      ...
      const productPromises = productIds.map(id => 
        apiClient.get<{success: boolean; data: Product}>(`/api/v1/products/${id}`)
      );
        ...
        const commissionPromises = productIds.map(async (id) => {
          try {
            ...
            if (user.role === 'SUPER_ADMIN' || user.role === 'ADMIN') {
              const response = await apiClient.get<CommissionData[]>(`/api/v1/commissions/insurance/${id}`);
              ...
              for (const term of commonTerms) {
                try {
                  ...
                  }>(`/api/v1/commissions/calculate/${id}?premium_term=${encodeURIComponent(term)}`);
