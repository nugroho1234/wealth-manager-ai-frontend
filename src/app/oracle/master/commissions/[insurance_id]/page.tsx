...
function CommissionEditContent() {
  ...
  // Debug: Log user role
  console.log('MASTER Commission Edit - User role:', user?.role);
  ...
  const company_name = searchParams.get('company') || 'Unknown Company';
  ...
  // Restrict access to MASTER and SUPER_ADMIN only
  if (user?.role !== UserRole.MASTER && user?.role !== UserRole.SUPER_ADMIN) {
    return (
      <Sidebar>
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600">Only Master and Super Administrators can manage commissions.</p>
          </div>
        </div>
      </Sidebar>
    );
...
export default function MasterCommissionEditPage() {
  return (
