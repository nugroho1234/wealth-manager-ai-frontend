...
export default function ProposalPreviewPage() {
        ...
        try {
          ...
          if (pageNumber === 3) {
            try {
              ...
              const statusResponse = await apiClient.get(`/api/v1/proposals/${proposalId}/intelligent-analysis-status`);
          ...
          const response = await apiClient.get(`/api/v1/proposals/${proposalId}/page/${pageNumber}`);
      ...
      try {
        ...
        if (pages.length >= 3 && proposalId) {
          ...
          if (status === 'completed' && pages[2].content.includes('Analyzing Cash Surrender Values')) {
            ...
            try {
              const page3Response = await apiClient.get(`/api/v1/proposals/${proposalId}/page/3`);
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const response = await apiClient.get(`/api/v1/proposals/${proposalId}/download-pdf`, {
        responseType: 'blob'
      });
