  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCompany) {
        ...
        response = await apiClient.patch(`/api/v1/companies/${editingCompany.company_id}`, payload);
        notifySuccess('Success', 'Company updated successfully');
      } else {
        // Create new company
        response = await apiClient.post('/api/v1/companies', payload);
  ...
  const handleLogoUpload = async (companyId: string) => {
    ...
    try {
      ...
      const response = await apiClient.post(
        `/api/v1/companies/${companyId}/upload-logo`,
        formData,
        {
          headers: {
